...
import { useAddComplianceScannerTypesMutation,useUpdateComplianceScannerTypesMutation } from "../../store/api/cyberService/ComplianceScannerTypesApi";

const LinkCS = ({ record, onClose, editRecord }) => {
  ...
  const [addComplianceScannerTypes, { isLoading }] = useAddComplianceScannerTypesMutation();
  const [updateComplianceScannerTypes] = useUpdateComplianceScannerTypesMutation();
  ...
  const handleCheckboxChange = (selectedRowKeys) => {
    setSelectedItems((prev) => ({
      ...prev,[activeTab]: [...selectedRowKeys],
    }));
  };

  const handleSubmit = async () => {
    const payload = {
      compliance_id: record._id,
      scanner_type_id: Object.values(selectedItems).flat(),
    };
    try {
      if (record?.compliance_scanner_mapping?._id) {
        await updateComplianceScannerTypes({
          complianceId: record.compliance_scanner_mapping._id,
          updatedData: payload,
        }).unwrap();
        message.success("Compliance scanner types updated successfully.");
      } else {
        await addComplianceScannerTypes(payload).unwrap();
        message.success("Scanner type added to compliance.");
      }
      ...
      message.error(error?.data?.error || "Failed to update compliance scanners.");
    }
  };
