...
import { useAddAsvsScannerTypesMutation,useUpdateAsvsScannerTypesMutation } from "../../store/api/cyberService/AsvsScannerTypesApi";

const LinkASVS = ({ record, onClose, editRecord,asvsScannerTypeData }) => {
  ...
  const [addAsvsScannerTypes, { isLoading }] = useAddAsvsScannerTypesMutation();
  const [updateAsvsScannerTypes] = useUpdateAsvsScannerTypesMutation();
...
const handleSubmit = async () => {
  ...
  try {
    const existingMapping = asvsScannerTypeData?.data?.find(
      (scanner) => scanner.asvs_id === record._id
    );

    if (existingMapping) {
      await updateAsvsScannerTypes({
        asvs_id: existingMapping._id,
        updatedData: payload,
      }).unwrap();
      message.success("ASVS scanner types updated successfully.");
    } else {
      await addAsvsScannerTypes(payload).unwrap();
      message.success("Scanner type added to ASVS.");
    }
    ...
    message.error(error?.data?.error || "Failed to update ASVS scanners.");
  }
};
  ...
  useEffect(() => {
    if (record?._id && asvsScannerTypeData?.data?.length > 0) {
      const preSelected = asvsScannerTypeData.data
      .filter((scanner) => scanner.asvs_id === record._id)
      .reduce((acc, scanner) => {
        if (Array.isArray(scanner.scanner_type_id)) { 
          scanner.scanner_type_id.forEach((id) => {
            const category = scanData?.data?.find((item) => item._id === id)?.scan_target_type;
            if (category) {
  ...
  }, [record, asvsScannerTypeData, scanData]);
